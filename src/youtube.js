let _speedUp = (ratio = "2", trial = 0) => {
  setting_button = document.getElementsByClassName(
    "ytp-button ytp-settings-button"
  )[0];
  setting_button.click();

  control_menu_buttons = Array.from(
...
let _skipAds = () => {
  skip_ads_button = document.getElementsByClassName(
    "ytp-ad-skip-button ytp-button"
  )?.[0];
  skip_ads_button?.click();
  close_ads_buttons = Array.from(
    document.getElementsByClassName("ytp-ad-overlay-close-button")
  );
...
let _changeViewMode = (mode, trial = 0) => {
  let full_screen_button = document.getElementsByClassName(
  ...
  let view_button = document.getElementsByClassName(
...
let applySetting = ({ video_setting }) => {
  ...
  let videoType = Array.from(document.getElementsByTagName("meta"))
    .filter((item) => item.getAttribute("itemprop") == "genre")[0]
